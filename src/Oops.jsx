...
import './App.css'
import { quizoops } from './assets/quiz2'
...
const nextQuestion=()=>{
  ...
  if(currentQuestion<quizoops.length-1){
...
// const selected=(ans)=>{
//  if(ans==quizoops[currentQuestion].answer){
...
const updateSCore=()=>{
  if(clickedOption==quizoops[currentQuestion].answer){
    setScore(score + 1)
  }
}
// console.log(score);
    ...
    <>
      <div className="container1">
        <div className="quiz-box">
          {result?
        <ScoreBoardj   score={score} totalScore={quizoops.length} />
        :
        (
          <>
          <img src={logo} alt="" />
         ...
         <span>{quizoops[currentQuestion].question}</span>
         </div>
         <div className="options">
          {quizoops[currentQuestion].options.map((option,index)=>(
